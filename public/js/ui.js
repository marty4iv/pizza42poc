...
/**
 * Enables the order button when the radio button is clicked
 */
const enableOrderButton = async () => {
...
const updateUI = async () => {
  try {
    ...
    // Enables the button to call the API
    //document.getElementById("btn-call-api").disabled = !isAuthenticated;

    if (isAuthenticated) {
      const user = await auth0.getUser();
      //Disable order button if email not validated
      document.getElementById("btn-submit-order").disabled = ! user.email_verified
